...
| [ATARI_ATASM_CIO_PUTCHEAT.asm](https://github.com/kenjennings/HelloWhirled/blob/master/ATARI_ATASM_CIO_PUTCHEAT.asm "ATARI_ATASM_CIO_PUTCHEAT.asm") | Uses the OS Central I/O in a slightly less than sanctioned way to write the characters to the screen editor (E: device.)  It reduces the IOCB set up effort by using the PUT CHAR vector shortcut in the IOCB channel which is intended for Atari BASIC. |
